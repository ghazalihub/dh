...
def predict_ortho_xgb(data_dict):
    """
    data_dict: dict containing the 6 biomechanical features
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    processed_data_path = os.path.join(base_dir, 'processed_data_xgb.pkl')
    best_model_path = os.path.join(base_dir, 'best_xgb_model.pkl')
    ...
    try:
        ...
        print(f"XGBoost model files not found. Please ensure 'best_xgb_model.pkl' and 'processed_data_xgb.pkl' are in {base_dir}")
        return None
...
def main():
    if len(sys.argv) == 7:
        try:
            ...
            result = predict_ortho_xgb(data)
            print(f"Prediction (XGBoost): {result}")
        except ValueError:
            print("Invalid input: Please provide 6 numeric biomechanical features.")
    else:
        print("Usage: python xgboost_experiment/ortho_ai_xgb.py <pelvic_incidence> <pelvic_tilt> <lumbar_lordosis_angle> <sacral_slope> <pelvic_radius> <degree_spondylolisthesis>")
